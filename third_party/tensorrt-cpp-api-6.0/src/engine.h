...
class Logger : public nvinfer1::ILogger {
public:
    void log(Severity in_severity, const char* in_msg) noexcept override {
        switch (in_severity) {
            case Severity::kINTERNAL_ERROR: LOG(FATAL) << in_msg; break;
            case Severity::kERROR: LOG(ERROR) << in_msg; break;
            case Severity::kWARNING: LOG(WARNING) << in_msg; break;
            case Severity::kINFO: LOG(INFO) << in_msg; break;
            case Severity::kVERBOSE: LOG(INFO) << in_msg; break;
            default: LOG(INFO) << in_msg; break;
        }
    }
};
